...
%DIF DEL pure_tetrad_arxiv_v3.tex   Fri May 23 22:51:03 2025
%DIF ADD pure_tetrad_arxiv_v4.tex   Thu Aug 28 10:45:51 2025
% Arsclassica Article
% LaTeX Template
% Version 1.1 (1/8/17)
%
...
\begin{document}
	...
	% (\section*)
	\noindent
	\textit{ \DIFdelbegin \DIFdel{Driven by the need for numerical solutions to the Einstein field equations,
	we derive }\DIFdelend \DIFaddbegin \DIFadd{This paper presents }\DIFaddend a \DIFaddbegin \DIFadd{derivation of a }\DIFaddend first-order reduction \DIFaddbegin \DIFadd{and 3+1
	decomposition }\DIFaddend of the \DIFdelbegin \DIFdel{second-order $ f(T) $-teleparallel
	gravity equations }\DIFdelend \DIFaddbegin \DIFadd{teleparallel equivalent of general relativity (TEGR) }\DIFaddend in
	the pure-tetrad formulation (no spin connection). \DIFdelbegin \DIFdel{We then restrict }\DIFdelend \DIFaddbegin \DIFadd{Our analysis demonstrates
	that in vacuum spacetimes, }\DIFaddend our \DIFdelbegin \DIFdel{attention to the teleparallel equivalent of general
	relativity (TEGR) and propose a }\DIFdelend 3+1 \DIFdelbegin \DIFdel{decomposition of these }\DIFdelend \DIFaddbegin \DIFadd{TEGR }\DIFaddend equations \DIFdelbegin \DIFdel{suitable for computational implementation. We demonstrate that in vacuum 	(matter-free spacetime) }\DIFdelend \DIFaddbegin \DIFadd{has }\DIFaddend the \DIFdelbegin \DIFdel{obtained system }\DIFdelend \DIFaddbegin \DIFadd{principal part }\DIFaddend of
	\DIFdelbegin \DIFdel{first-order equations is
	}\DIFdelend \DIFaddbegin \DIFadd{the differential operator }\DIFaddend equivalent to the \DIFaddbegin \DIFadd{one of }\DIFaddend tetrad reformulation of
	general relativity by Estabrook, Robinson, Wahlquist, and Buchman and
	Bardeen, and therefore \DIFaddbegin \DIFadd{the presented 3+1 decomposition of TEGR }\DIFaddend also admits a
	symmetric hyperbolic formulation\DIFdelbegin \DIFdel{. However}\DIFdelend , \DIFdelbegin \DIFdel{the question }\DIFdelend \DIFaddbegin \DIFadd{a desirable property for ensuring
	well-posedness }\DIFaddend of \DIFdelbegin \DIFdel{hyperbolicity of
	}\DIFdelend the \DIFdelbegin \DIFdel{3+1 TEGR equations for arbitrary spacetimes remains unaddressed so far}\DIFdelend \DIFaddbegin \DIFadd{initial value problem}\DIFaddend . Furthermore, the structure of
...
\cite{Capozziello2021,Pati2022}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Maluf2001a,Capozziello2021,Pati2022}}\hskip0pt%DIFAUXCMD
}\DIFaddend . A Hamiltonian formulation of
	TEGR was used in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Pati2022} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Maluf2001a,Pati2022} }\hskip0pt%DIFAUXCMD
}\DIFaddend to obtain evolution equations
	...
	to the foliations were chosen as the state variables. \DIFdelbegin \DIFdel{Here, }\DIFdelend \DIFaddbegin \DIFadd{Somewhat similar to
	the Hamiltonian formalism in systematic use of conjugate variables and
	Legendre transforms, here }\DIFaddend we explore another line of deriving \DIFaddbegin \DIFadd{a }\DIFaddend 3+1
	formulation of TEGR \DIFdelbegin \DIFdel{which is rather }\DIFdelend \DIFaddbegin \DIFadd{that is }\DIFaddend aligned with the relativistic
	electrodynamics. \DIFaddbegin \DIFadd{In particular, the key difference from the Hamiltonian
	formulations \mbox{%DIFAUXCMD
\cite{Maluf2001a,Pati2022} }\hskip0pt%DIFAUXCMD
is in the promotion of the torsion
	to the dynamical variable with its own evolution equation. }\DIFaddend As the result,
	the obtained \DIFdelbegin \DIFdel{equations are complitelly
	}\DIFdelend \DIFaddbegin \DIFadd{3+1 equations are completely }\DIFaddend different from the mentioned
	papers \DIFaddbegin \DIFadd{and have a rather elegant structure similar to Maxwell's equations}\DIFaddend .
	...
	and \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend 3+1 decomposition is performed. \DIFdelbegin \DIFdel{However, despite this independent
	route
	}\DIFdelend \DIFaddbegin \DIFadd{Although, completely different route
	have been taken to obtain our 3+1 TEGR equations}\DIFaddend , we demonstrate that the
	resulting \DIFdelbegin \DIFdel{3+1 }\DIFdelend equations are equivalent to \DIFdelbegin \DIFdel{tetrad reformulations of the GRsuch as the one by
	Estabrook-Robinson-Wahlquist \mbox{%DIFAUXCMD
\cite{Estabrook1997} }\hskip0pt%DIFAUXCMD
and Buchman-Bardeen
	\mbox{%DIFAUXCMD
\cite{Buchman2003} }\hskip0pt%DIFAUXCMD
(the \ERWBB\ formulation for brevity), and recent dGREMformulation of GR }\DIFdelend \DIFaddbegin \DIFadd{the recently proposed tetrad
	reformulation of GR, called dGREM, }\DIFaddend \cite{Olivares2022}.
	...
	particular, for \DIFdelbegin \DIFdel{a vacuum space-time}\DIFdelend \DIFaddbegin \DIFadd{vacuum spacetimes and a certain choice of gauge conditions}\DIFaddend ,
	we have found that the proposed 3+1 TEGR equations \DIFdelbegin \DIFdel{(if written in the tetrad frame) are
	equivalent to ERWBB \mbox{%DIFAUXCMD
\cite{Estabrook1997,Buchman2003} }\hskip0pt%DIFAUXCMD
which is known to by symmetric hyperbolic for
	a certain choice of gauge conditions}\DIFdelend \DIFaddbegin \DIFadd{can be transformed into a
	symmetric hyperbolic system similar to the tetrad reformulation of the GR
	proposed by Estabrook, Robinson, and Wahlquist \mbox{%DIFAUXCMD
\cite{Estabrook1997} }\hskip0pt%DIFAUXCMD
and
	further developed by Buchman and Bardeen
	\mbox{%DIFAUXCMD
\cite{Buchman2003,Buchman2005,Bardeen2011} }\hskip0pt%DIFAUXCMD
(the \ERWBB\ formulation for
	brevity)}\DIFaddend . 
	...
	\cite{Golovnev2017a}, the spin connection \DIFdelbegin \DIFdel{of TEGR }\DIFdelend introduces extra degrees of
	freedom which do not have evolution equations \DIFaddbegin \DIFadd{in TEGR }\DIFaddend and therefore can be
	treated as \DIFdelbegin \DIFdel{parameters (not state variables) }\DIFdelend \DIFaddbegin \DIFadd{non-dynamical parameters }\DIFaddend of the theory. Since we are
	...
	\DIFdelbegin \DIFdel{$
	3+1 $ }\DIFdelend \DIFaddbegin \DIFadd{3+1 }\DIFaddend equations we obtain in this paper resemble very closely the structure
	...
	\DIFaddbegin \DIFadd{This work is organized as follows. We start with recalling important
	...
	transforming these equations into an equivalent symmetric hyperbolic system similar to that of \ERWBB\ \mbox{%DIFAUXCMD
\cite{Estabrook1997,Buchman2003}}\hskip0pt%DIFAUXCMD
. Finally, we
	...
	\DIFaddend \section{Definitions}\DIFaddbegin \label{sec.def}
	\DIFaddend 
	...
	We shall also need the Levi-Civita symbol (tensor-density\footnote{We 
		...
		under a 
		general 
		coordinate change $ x^\mu \to x^{\mu'} $\DIFaddbegin \DIFadd{, }\DIFaddend the determinant $ \det(\tetr{a}{\mu}) = \detTetr $ 
	...
	\section{Variational formulation}\DIFaddbegin \label{sec.SAP}
	\DIFaddend 
	...
	\Lag $, so that in principle \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{some elements of our }\DIFaddend derivation can be adopted
	for the extensions of the teleparallel gravity such as $ f(\Tscal)
	$-teleparallel gravity \DIFdelbegin \DIFdel{, 
	where $ f(\Tscal) $ is }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{li2011d,li2011e,ferraro2018,blagojevic2020}}\hskip0pt%DIFAUXCMD
, in
	which the Lagrangian $ \Lag = f(\Tscal) $ is defined as }\DIFaddend some function of the
	torsion scalar $ \Tscal $, see Section\,\ref{sec.closure}. We shall utilize
	the explicit form of the TEGR Lagrangian only in the last part of the paper\DIFdelbegin \DIFdel{.
	...
	In the setting of \DIFdelbegin \DIFdel{the }\DIFdelend teleparallel gravity, $ \Lag $ is not a function of a general combination 
	of 
	...
	\begin{subequations}
		\begin{align}
			\Laghodge_{\HDT{a\mu\nu}}u^\nu &= -\frac{1}{2}\left( \LagBE_{\BT{a}{\mu}} +u_\mu 
			\LagBE_{\BT{a}{\lambda}} u^\lambda \right), \label{eqn.1st.order.EL}
			\\[2mm]
			\Lagtors_{\Tors{a}{\mu\nu}}u_\nu &= -\frac{1}{2}\left( \LagBE_{\ET{a}{\mu}} + u^\mu 
	...
	\section{Summary of the 3+1 TEGR equations}\DIFaddbegin \label{sec.summary}
	\DIFaddend Here, we summarize the 3+1 TEGR equations and give explicit expressions for the 
	constitutive 
	...
	%DIF < 	\begin{align}
		%DIF < 		\Hfin{\sA}{k} := %\Ufin_{\Bfin{\sA}{k}} =
		%DIF < 		-\frac{\alpha}{\varkappa \detTetr_3} 
		%DIF < 		\mg{\sA\sB}\mg{\sC\sD} 
		%DIF < 		& \left( \tetr{\sD}{k} \tetr{\sB}{j} - \frac12 \tetr{\sD}{j} \tetr{\sB}{k} \right) 
		%DIF < 		\Bfin{\sC}{j}
		%DIF < 		- 
		%DIF < 		\frac{1}{\varkappa \detTetr_3} \mg{\sA,\sB} \LCsymb^{ijl} \gamma_{ik} \tetr{\sB}{j} 
		%DIF < 		\Efin{\indalg{0}}{l}
		%DIF < 		%\frac{\detTetr_3}{\varkappa} \LCsymb_{kli} \itetr{l}{\sA} \gamma^{ij} 
		%DIF < 		%\Efin{\indalg{0}}{j}
		%DIF < 		,\\[2mm]
		%DIF < 		\Efin{\sA}{k} := %\Ufin_{\Dfin{\sA}{k}}=
		%DIF < 		-\frac{\alpha \varkappa}{\detTetr_3} 
		%DIF < 		& \left( \tetr{\sA}{j} \tetr{\sB}{k} - \frac12 \tetr{\sA}{k} \tetr{\sB}{j} \right) 
		%DIF < 		\Dfin{\sB}{j} 
		%DIF < 		- 
		%DIF < 		\frac{\varkappa}{\detTetr_3} \LCsymb^{ijl} 
		%DIF < 		\gamma_{ik} \tetr{\sA}{j} \Hfin{\indalg{0}}{l}.
		%DIF < 	\end{align}
	%DIF < \end{subequations}
	\DIFaddbegin 

	\DIFaddend with the total (matter+gravity) momentum $ \rho_{\sA} = \itetr{i}{\sA}\rho_i $ and the total 
...
%DIF < 	\begin{gather*}
%DIF < 		\stress{i}{k} := -\Bfin{a}{k} 
%DIF < 		\Hfin{a}{i} - 
%DIF < 		\Dfin{a}{k} \Efin{a}{i} + (\Bfin{a}{j} 
%DIF < 		\Hfin{a}{j} + 
%DIF < 		\Dfin{a}{j} \Efin{a}{j} 
%DIF < 		- \Ufin)\delta^k_{\ i},
%DIF < 	\end{gather*}
	%DIF < \begin{align*}
	%DIF < 	\rhs{i} =&-\rho_k \itetr{k}{\sA} \Efin{\sA}{i} - \LCsymb_{ilj} s^l_{\ k} \itetr{k}{\sA} 
	%DIF < 	\Bfin{\sA}{j} + s^l_{\ k} \itetr{k}{\sA} \pd{i}\tetr{\sA}{l} + \rho_j 
	%DIF < \pd{i}\shift{j},\\[2mm]
	%DIF < 	\rhs{0} =&- \left(\pd{t} \ln(\alpha) - \shift{k} \pd{k}\ln(\alpha) \right) \Ufin
	%DIF < 	+ \left( \pd{t} \shift{\sA} - \shift{k} \pd{k}\shift{\sA} \right) \rho_j 
	%DIF < 	\itetr{j}{\sA}
	%DIF < 	+ \LCsymb^{klj} \Hfin{a}{l}\Efin{a}{j}\pd{k}\ln(\alpha) + s^k_{\ j} \itetr{j}{\sA} 
	%DIF < 	\pd{k}\shift{\sA}.
	%DIF < \end{align*}
	%DIF < while the source terms in the energy-momentum equations read
	%DIF < \begin{align*}
	%DIF < 	\rhs{i} =&-\rho_k \itetr{k}{\sA} \Efin{\sA}{i} + \rho_j \pd{i}\shift{j}
	%DIF < 	+\stress{j}{k} \w{j}{ki}\\[2mm]
	%DIF < %	-\rho_\sA  \Efin{\sA}{i} - \LCsymb_{ilj} s^l_{\ \sA} 
	%DIF < %	\Bfin{\sA}{j} + \rho_\sA \pd{i}\shift{\sA} + (-\shift{k} \rho_{\sA}+ s^k_{\ \sA})  
	%DIF < %	\pd{i}\tetr{\sA}{k} = 
	%DIF < %	-\rho_\sA  \Efin{\sA}{i} - \LCsymb_{ilj} s^l_{\ \sA} \Bfin{\sA}{j}
	%DIF < %	+ \rho_k\pd{i}\shift{k} + s^k_{\ j} \w{j}{ik},\\[2mm]
	%DIF < 	%
	%DIF < 	\rhs{0} =&
	%DIF < 	- \left(\pd{t} \ln(\alpha) - \shift{k} \pd{k}\ln(\alpha) \right) \Ufin
	%DIF < 	+ \left( \pd{t} \shift{\sA} - \shift{k} \pd{k}\shift{\sA} \right) \rho_{\sA}
	%DIF < 	+ \LCsymb^{klj} \Hfin{\sA}{l}\Efin{\sA}{j}\pd{k}\ln(\alpha) + \stress{\sA}{k} 
	%DIF < 	\pd{k}\shift{\sA}\\[2mm]
	%DIF < 	=&
	%DIF < 	- \left(\pd{t} \ln(\alpha) - \shift{k} \pd{k}\ln(\alpha) \right) \Ufin
	%DIF < 	+ \rho_j \pd{t} \shift{j} + \stress{j}{k} \pd{k}\shift{j} + \rho_{\sA} \shift{j} 
	%DIF < \Efin{\sA}{j} 
	%DIF < 	+ \LCsymb^{klj} \Hfin{\sA}{l}\Efin{\sA}{j}\pd{k}\ln(\alpha) + \shift{j} \stress{l}{k} 
	%DIF < \w{l}{kj},
	%DIF < \end{align*}
	%DIF < where $ \rho_{\sA} = \itetr{i}{\sA} \rho_i$, 
	%DIF < %$ s^k_{\ \sA} = \itetr{i}{\sA} s^k_{\ i} $, 
	%DIF < and $ 
	%DIF < \w{j}{ik} = \itetr{j}{\sA}\pd{i}\tetr{\sA}{k}$.
	...
	\begin{subequations}\label{eqn.H.E}
		\begin{multline}
			...
			- \frac{1}{2} \tetr{\sD}{j} \tetr{\sB}{k} \right) 
			\Bfin{\sC}{j}
			\\
			- 
		...
		\begin{multline}
			...
			-\frac{\alpha \varkappa}{\detTetr_3} 
			\left( \tetr{\sA}{j} \tetr{\sB}{k} - \frac{1}{2} \tetr{\sA}{k} \tetr{\sB}{j} \right) 
			\Dfin{\sB}{j} 
			\\
			- 
	...
	%DIF < Noether current \eqref{eqn.J} becomes:
	%DIF < \begin{subequations}
	%DIF < \begin{equation}\label{eqn.J_Ai}
	%DIF < 	\NC{\sA}{i} =-\nc{\sA}{i} 
	%DIF < 	-\frac{1}{2\alpha}  \beta^i \left( \tetr{\sB}{j} \itetr{k}{\sA} \Dfin{\sB}{j} 
	%DIF < 	- \frac{\detTetr_3}{\alpha \varkappa} \itetr{j}{\sA} 
	%DIF < 	\itetr{k}{\sB} \Efin{\sB}{j} \right) \Efin{\indalg{0}}{k}
	%DIF < 	+ \beta^i \LCsymb_{jkl} \itetr{j}{\sA} \Bfin{\sB}{k} \Dfin{\sB}{l},
	%DIF < \end{equation}
	%DIF < 
	%DIF < \begin{equation}\label{eqn.J_A0}
	%DIF < 	\NC{\sA}{0} =-\nc{\sA}{0}
	%DIF < 	+ \frac{1}{2\alpha} 
	%DIF < 	\left(
	%DIF < 	\itetr{i}{\sA} \tetr{\sB}{k} \Dfin{\sB}{k} - \frac{\detTetr_3}{\alpha \varkappa} 
	%DIF < \itetr{k}{\sA} 
	%DIF < 	\itetr{i}{\sB} \Efin{\sB}{k} 
	%DIF < 	\right) \Efin{\indalg{0}}{i} - \LCsymb_{ijl} \itetr{i}{\sA} \Bfin{\sB}{j} \Dfin{\sB}{l},
	%DIF < \end{equation}
	%DIF < 
	%DIF < \begin{align}\label{eqn.J_0i}
	%DIF < 	\NC{\indalg{0}}{i} =-\nc{\indalg{0}}{i}
	%DIF < 	&+ \frac{1}{\alpha} 
	%DIF < 	\left(
	%DIF < 	  \Bfin{\sB}{i} \Hfin{\sB}{0}
	%DIF < 	+ \beta^i \Bfin{\sB}{l} \Hfin{\sB}{l} 
	%DIF < 	- \beta^l \Bfin{\sB}{i} \Hfin{\sB}{l} \right) \nonumber\\
	%DIF < 	& + \frac{1}{\alpha} \beta^i
	%DIF < 	\left(
	%DIF < 	  \Dfin{\sB}{l} \Efin{\sB}{l} 
	%DIF < 	+ \Dfin{\indalg{0}}{l} \Efin{\indalg{0}}{l} 
	%DIF < 	\right) \nonumber\\ 
	%DIF < 	& + \frac{\detTetr_3}{2\alpha\varkappa}
	%DIF < 	\left( \itetr{k}{\sB} \gamma^{ij} - \itetr{i}{\sB} \gamma^{kj} \right) \Efin{\sB}{j} 
	%DIF < 	\Efin{\indalg{0}}{k} \nonumber\\
	%DIF < 	& + \frac{1}{\alpha} \LCsymb^{ilk} \Hfin{\sB}{l} \Efin{\sB}{k},
	%DIF < \end{align}
	%DIF < 
	%DIF < \begin{equation}\label{eqn.J_00}
	%DIF < 	\NC{\indalg{0}}{0} =-\nc{\indalg{0}}{0} 
	%DIF < 	- \frac{1}{\alpha} 
	%DIF < 	\left( 
	%DIF < 	  \Bfin{\sA}{i} \Hfin{\sA}{i} 
	%DIF < 	+ \Dfin{\sA}{i} \Efin{\sA}{i}
	%DIF < 	\right)
	%DIF < 	+ \frac{\detTetr_3}{\alpha \varkappa} \LCsymb_{ijk} \gamma^{il} \itetr{j}{\sA}\Bfin{\sA}{k} 
	%DIF < 	\Efin{\indalg{0}}{l}
	%DIF < \end{equation}
	%DIF < 
	%DIF < \end{subequations}
	%DIF < 
	%DIF < \begin{subequations}
	%DIF < 	\begin{align}\label{eqn.j_Ai}
		%DIF < 		\nc{\sA}{i} =&
		%DIF < 		\frac{\detTetr}{2\varkappa}\Bigg[
		%DIF < 		\phantom{+} \frac{2}{\alpha^2} \Bigg( 
		%DIF < 		\frac{1}{4} \left( \itetr{j}{\sA}\gamma^{ki} - 
		%DIF < 		\itetr{k}{\sA} 
		%DIF < 		\gamma^{ij} \right)  
		%DIF < 		\Efin{\indalg{0}}{j}\Efin{\indalg{0}}{k} %\nonumber\\
		%DIF < 		%
		%DIF < 		%&\phantom{ \frac{2}{\alpha^2} \Bigg(} 
		%DIF < 		+ 
		%DIF < 		\frac{1}{\alpha} \shift{i} 
		%DIF < 		\itetr{jk}{\sC\sA} \Efin{\sC}{j} \Efin{\indalg{0}}{k}
		%DIF < 		-	
		%DIF < 		\itetr{i}{\sC} \itetr{kj}{\sB\sA} \Efin{\sC}{j}\Efin{\sB}{k}
		%DIF < 		+
		%DIF < 		\frac{1}{2} \mg{\sC\sB}\gamma^{ik}\itetr{j}{\sA} \Efin{\sC}{j}\Efin{\sB}{k}
		%DIF < 		\Bigg) 
		%DIF < 		\nonumber\\
		%DIF < 		%
		%DIF < 		&\phantom{\phantom{+} \frac{2}{\alpha^2} \Bigg(}
		%DIF < 		+ \frac{2}{\detTetr_3^2} 
		%DIF < 		\Bigg( 
		%DIF < 		\tetr{\sD}{k}\mg{\sA\sB\sC\sD} - \itetr{i}{\sA} \tetr{\sE\sD}{kj} 
		%DIF < \mg{\sC\sE}\mg{\sB\sD}
		%DIF < 		\Bigg) \Bfin{\sB}{k}\Bfin{\sC}{i} 
		%DIF < 		\nonumber\\
		%DIF < 		%
		%DIF < 		&\phantom{\phantom{+} \frac{2}{\alpha^2} \Bigg(}
		%DIF < 		- \frac{2}{\alpha} \LCsymb_{jnk} 
		%DIF < 		\left(
		%DIF < 		\gamma^{in}\itetr{l}{\sA}\itetr{j}{\sC} +
		%DIF < 		\gamma^{il}\itetr{n}{\sA}\itetr{j}{\sC} -
		%DIF < 		\gamma^{jl}\itetr{n}{\sA}\itetr{i}{\sC}
		%DIF < 		\right) \Bfin{\sC}{k} \Efin{\indalg{0}}{l} \Bigg]
		%DIF < 		+ (\Dfin{a}{k} \Efin{a}{k} - \Ufin) \itetr{i}{\sA}
		%DIF < 	\end{align}
	%DIF < where we have used the following notations
	%DIF < 	\begin{equation}
		%DIF < 		\itetr{jk}{\sC\sA} = \itetr{j}{\sC} \itetr{k}{\sA} - \frac12 \itetr{j}{\sA} 
		%DIF < \itetr{k}{\sC},
		%DIF < 		\quad
		%DIF < 		\tetr{\sC\sD}{kj} = \tetr{\sC}{k}\tetr{\sD}{j} - \frac12 \tetr{\sC}{j}\tetr{\sD}{k},
		%DIF < 		\quad
		%DIF < 		\mg{\sA\sB\sC\sD} = \mg{\sA\sB} \mg{\sC\sD} - \frac12 \mg{\sA\sC} \mg{\sB\sD}.
		%DIF < 	\end{equation}
	%DIF < \end{subequations}
...
%DIFDELCMD < 	
%DIFDELCMD < 	%%%
\DIFdelend \subsection{Stationary differential constraints}
	...
	%DIF < \subsubsection{Free parameters}
	%DIF < 
	%DIF < The lapse $\alpha$  an shift vector $\shift{k}$ are not subjected to any constraints and can 
	%DIF < be 
	%DIF < evolved in an arbitrary but meaningful way. 
	\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < 	%%%
%DIF < $ \LCsymb_{\sA\sB\sC} = \LCsymb_{ijk} \itetr{i}{\sA}\itetr{j}{\sB}\itetr{k}{\sC} $
	%DIFDELCMD < 

%DIFDELCMD < 	%%%
%DIF < \subsection{Summary of change of variables and potentials}
	%DIF < 
	%DIF < \begin{itemize}
	%DIF < 	\item[$ \left\langle 1 \right\rangle $] 
	%DIF < 	$ \Lag(\tetr{a}{\mu},\pd{\lambda}\tetr{a}{\nu}) = 
	%DIF < 	\Laghodge(\tetr{a}{\mu},\HDT{a\mu\nu}) $,  
	%DIF < 	\quad 
	%DIF < 	$ \{\tetr{a}{\mu},\pd{\mu}\tetr{a}{\nu}\} \to \{\tetr{a}{\mu},\HDT{a\mu\nu}\}$,
	%DIF < 	\quad
	%DIF < 	eqs. \eqref{eqn.EL}$ \to $ \eqref{eqn.1st.order.TEGR}
	%DIF < 	\\[1mm]
	%DIF < 	%
	%DIF < 	\item[$ \left\langle 2 \right\rangle $] 
	%DIF < 	$ \Laghodge(\tetr{a}{\mu},\HDT{a\mu\nu}) = 
	%DIF < 	\LagBE(\tetr{a}{\mu},\BT{a}{\mu},\ET{a}{\nu})$,
	%DIF < 	\quad
	%DIF < 	$ \{\tetr{a}{\mu},\HDT{a\mu\nu}\} \to \{\tetr{a}{\mu},\BT{a}{\mu},\ET{a}{\nu}\} $,
	%DIF < 	\quad
	%DIF < 	eqs. \eqref{eqn.1st.order.TEGR}$ \to $\eqref{eqn.tors.BE}
	%DIF < 	\\[1mm]
	%DIF < 	%
	%DIF < 	\item[$ \left\langle 3 \right\rangle $] 
	%DIF < 	$ \LagBE(\tetr{a}{\mu},\BT{a}{\mu},\ET{a}{\nu}) \to 
	%DIF < 	\Um(\tetr{a}{\mu},\Bm{a}{\mu},\Dm{a}{\mu})$,
	%DIF < 	\quad
	%DIF < 	$ \{\tetr{a}{\mu},\BT{a}{\mu},\ET{a}{\nu}\} \to \{\tetr{a}{\mu},\Bm{a}{\mu},\Dm{a}{\mu}\}$,
	%DIF < 	\quad
	%DIF < 	eqs. \eqref{eqn.tors.BE}$ \to $\eqref{eqn.PDE.4D}
	%DIF < 	\\[1mm]
	%DIF < 	%
	%DIF < 	\item[$ \left\langle 4 \right\rangle $] 
	%DIF < 	$ \Um(\tetr{a}{\mu},\Bm{a}{\mu},\Dm{a}{\mu}) = \aU(\tetr{a}{\mu},\aB{a}{\mu},\aD{a}{\mu})$,
	%DIF < 	\quad
	%DIF < 	$\{\tetr{a}{\mu},\Bm{a}{\mu},\Dm{a}{\mu}\} \to 
	%DIF < 	\{\tetr{a}{\mu},\aB{a}{\mu},\aD{a}{\mu}\}$,
	%DIF < 	\quad
	%DIF < 	eqs. \eqref{eqn.PDE.4D}$ \to $\eqref{eqn.tpo.2}
	%DIF < 	\\[1mm]
	%DIF < 	%
	%DIF < 	\item[$ \left\langle 5 \right\rangle $] 
	%DIF < 	$ \aU(\tetr{a}{\mu},\aB{a}{\mu},\aD{a}{\mu}) = 
	%DIF < 	\Ufin(\tetr{a}{\mu},\Bfin{a}{\mu},\Dfin{a}{\mu})$,
	%DIF < 	\quad
	%DIF < 	$\{\tetr{a}{\mu},\aB{a}{\mu},\aD{a}{\mu}\} \to 
	%DIF < 	\{\tetr{a}{\mu},\Bfin{a}{\mu},\Dfin{a}{\mu}\}$,
	%DIF < 	\quad
	%DIF < 	eqs. \eqref{eqn.tpo.2}$ \to $\eqref{eqn.3+1.summary}
	%DIF < \end{itemize}
	%DIFDELCMD < 
...
%DIFDELCMD < 	
%DIFDELCMD < 	%%%
\DIFdelend \section{Hyperbolicity of the vacuum 3+1 TEGR equations}\DIFaddbegin \label{sec.hyperbolicity}
\DIFaddend 

	\DIFdelbegin \DIFdel{The 3+1 TEGR system \eqref{eqn.3+1.summary} is an overdetermined system due
	}\DIFdelend \DIFaddbegin \DIFadd{Due to }\DIFaddend the presence of the \DIFaddbegin \DIFadd{total (gravity + matter) }\DIFaddend momentum and energy
	equations \eqref{eqn.3+1.m} and \eqref{eqn.3+1.e}\DIFdelbegin \DIFdel{. The question }\DIFdelend \DIFaddbegin \DIFadd{, analysis }\DIFaddend of hyperbolicity
	of \DIFdelbegin \DIFdel{\eqref{eqn.3+1.summary},
	therefore, requires considering }\DIFdelend \DIFaddbegin \DIFadd{the 3+1 TEGR system \eqref{eqn.3+1.summary} requires consideration of }\DIFaddend a
	model for matter, which \DIFaddbegin \DIFadd{however }\DIFaddend goes beyond the scope of this paper.
	\DIFdelbegin \DIFdel{However}\DIFdelend \DIFaddbegin \DIFadd{Nevertheless}\DIFaddend , we can still analyze \DIFdelbegin \DIFdel{the }\DIFdelend hyperbolicity of the vacuum \DIFdelbegin \DIFdel{equations \eqref{eqn.3+1.summary} without }\DIFdelend \DIFaddbegin \DIFadd{TEGR
	equations \eqref{eqn.3+1.summary} for which }\DIFaddend the momentum and energy
	equations \DIFdelbegin \DIFdel{which might be considered as a starting point of further research. In particular}\DIFdelend \DIFaddbegin \DIFadd{can be omitted. This would be still an important result since in
	the empty space the field equations must be causal and have well-posed
	initial value problem. 
	}

	\DIFadd{In empty space, the 3+1 TEGR system reduces to the evolution equations
	...
	analysis. Moreover}\DIFaddend , we shall \DIFdelbegin \DIFdel{show that the }\DIFdelend \DIFaddbegin \DIFadd{assume that the lapse $\lapse$ and shift
	$\shift{i}$ are not dynamical but some prescribed functions of space and
	time.
}

	\DIFadd{We will show how the }\DIFaddend vacuum 3+1 TEGR equations\DIFdelbegin \DIFdel{are
	equivalent to the }\DIFdelend \DIFaddbegin \DIFadd{, when restricted to
	...
	first-order }\DIFaddend tetrad reformulation of GR \DIFdelbegin \DIFdel{by Estabrook-Robinson-Wahlquist
	\mbox{%DIFAUXCMD
\cite{Estabrook1997} }\hskip0pt%DIFAUXCMD
and Buchman-Bardeen \mbox{%DIFAUXCMD
\cite{Buchman2003} }\hskip0pt%DIFAUXCMD
which }\DIFdelend \DIFaddbegin \DIFadd{developed by Estabrook, Robinson, and
	Wahlquist \mbox{%DIFAUXCMD
\cite{Estabrook1997} }\hskip0pt%DIFAUXCMD
and by Buchman and Bardeen
	\mbox{%DIFAUXCMD
\cite{Estabrook1997}}\hskip0pt%DIFAUXCMD
. This specific formulation, which we will refer to as
	the \ERWBB\ formulation, }\DIFaddend is known to be \DIFdelbegin \DIFdel{symmetric hyperbolic, and will be referenced to as }\DIFdelend \DIFaddbegin \textit{\DIFadd{symmetric hyperbolic}} \DIFadd{if
	...
	\eqref{eqn.3+1.h} and the new system having the form of the }\DIFaddend \ERWBB\
	formulation \DIFdelbegin \DIFdel{. We }\DIFdelend \DIFaddbegin \DIFadd{are not exactly equivalent. In other words, in the context of
	numerical relativity, this may require a constraint-compatible
	discretization, e.g. see \mbox{%DIFAUXCMD
\cite[Sec.VI.G]{Olivares2022} }\hskip0pt%DIFAUXCMD
or
	\mbox{%DIFAUXCMD
\cite{oliynyk2025}}\hskip0pt%DIFAUXCMD
.
}

	\DIFadd{Moreover, we could not confirm or disprove the equivalence of the TEGR
	...
	\DIFadd{Let us also }\DIFaddend remind that solely hyperbolicity is not enough for
	...
	standard 
	covariant derivative of GR \DIFdelbegin \DIFdel{associated with }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend the symmetric Levi-Civita connection, and $ 
	\Gamma^{\mu}_{\ 
		\nu\lambda} $ are the Christoffel symbols of the Levi-Civita connection.

	If $ \Tors{a}{bc} = \Tors{a}{\mu\nu}\itetr{\mu}{b}\itetr{\nu}{c} $ and $ \Tors{}{abc} = \mg{ad} 
	\Tors{d}{bc} $ is the torsion \DIFaddbegin \DIFadd{coefficients if it is }\DIFaddend written in the tetrad basis, then the relation between the Ricci 
	rotation \DIFdelbegin \DIFdel{coefficient }\DIFdelend \DIFaddbegin \DIFadd{coefficients }\DIFaddend and torsion can be expressed by the formula
	...
	\DIFaddbegin \DIFadd{Additionally, introducing the vector
	}\begin{equation}
		\DIFadd{n^\sA = \frac{1}{2} \LCtens^{\sA\sB\sC} \mg{\sC\sD} \Nbuch{\sB}{\sD},
	}\end{equation}
	\DIFadd{the Hamiltonian constraint in terms of \ERWBB\ takes the form (see \mbox{%DIFAUXCMD
\cite[eq.(A1)]{Buchman2003}}\hskip0pt%DIFAUXCMD
)
	}\begin{multline}\DIFadd{\label{eqn.Hamiltonian.ERWBB}
		\pd{\sA} n^\sA = \frac{1}{2} \Nbuch{\sA\sB}{}\Nbuchmix{\sA\sB}{} + \frac{1}{4}\left( \Kbuchmix{\sA\sB}{}\Kbuch{\sA}{\sB} - \Nbuch{\sA\sB}{}\Nbuchmix{\sB\sA}{} \right) 
		}\\
		\DIFadd{- \frac{1}{4}\left( \left( \Kbuchmix{\sA}{\sA} \right)^2 + \left( \Nbuch{\sA}{\sA} \right)^2\right),
	}\end{multline}
	\DIFadd{where $\pd{\sA} = \itetr{k}{\sA}\pd{k}$.
}

	\DIFaddend It can be shown that the following \DIFaddbegin \DIFadd{one-to-one }\DIFaddend relations between the \ERWBB\ and TEGR state variables hold
	\begin{subequations}\label{eqn.Buchman.TEGR.KN}
		...
		\end{align}
	\end{subequations}
	\DIFaddbegin \DIFadd{where $\Dfin{\sA\sB}{} = \mg{\sA\sC}\tetr{\sB}{i}\Dfin{\sB}{i}$ and $\Bfinmixx{\sA}{\sB} = \mg{\sA\sC}\tetr{\sB}{i}\Bfin{\sC}{i}$. }\DIFaddend We remark that due to our choice of the 3+1 split \eqref{eqn.h.eta.matrix},
	i.e. that observer's time vector, $ \bas{\indalg{0}} $, is \DIFdelbegin \DIFdel{align }\DIFdelend \DIFaddbegin \DIFadd{aligned }\DIFaddend with the
	...
	a_{\sA} $, $ \omega^{\sA} $ and the \tegr\ vectors \DIFdelbegin \DIFdel{$
	...
	}\DIFaddend \begin{equation}\label{eqn.Buchman.TEGR.AW}
		a_{\sA} = \lapse^{-1} A_{\sA},
		\qquad
	...
	%DIF <  Unfortunately, system \eqref{eqn.3+1.summary} is not immediately hyperbolic
	%DIF <  even for the Minkowski spacetime (it is actually only weakly hyperbolic
	%DIF <  because some eigenvectors are missing), and therefore it can not be
	%DIF <  equivalent to the symmetric hyperbolic \ERWBB\ formulation of GR right away.
	%DIF <  In this section, we discuss what modifications of equations
	%DIF <  \eqref{eqn.3+1.summary} are necessary in order to make it equivalent to the
	%DIF <  \ERWBB\ formulation. These modifications rely on the use of Hamiltonian and
	%DIF <  momentum constraints \eqref{eqn.div.constrD} and symmetry of $
	%DIF <  \Dfin{\sA\sB}{} $. 
	\DIFaddbegin \DIFadd{First of all, let us note that, in the tetrad frame, the tetrad equation \eqref{eqn.3+1.h} reads
	}\begin{equation}
		\DIFadd{\pd{\indalg{0}} \itetr{k}{\sA} = \frac{1}{\lapse} \left( \Efin{\sA}{i}\itetr{i}{\sB}\itetr{k}{\sA} - \pd{\sA} \shift{k} \right),
	}\end{equation}
	\DIFadd{where $ \pd{\indalg{0}} = \itetr{\mu}{\indalg{0}}\pd{\mu} = u^\mu\pd{\mu} =
	...
	%DIF <  Because we are considering a vacuum space-time, the momentum and energy
	%DIF <  equations \eqref{eqn.3+1.m}, \eqref{eqn.3+1.e} can be omitted. Moreover,
	%DIF <  modifications concern only torsion equations \eqref{eqn.3+1.D},
	%DIF <  \eqref{eqn.3+1.B}, and therefore for the sake of simplicity, we also omit
	%DIF <  the tetrad equations \eqref{eqn.3+1.h}
	\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < 	%%%
%DIF <  If one considers 18 equations \eqref{eqn.3+1.D}, \eqref{eqn.3+1.B} written in a 
	%DIF <  quasilinear form\footnote{Due to the rotational invariance, 
	%DIF <  	it is actually sufficient to consider the system in any preferred coordinate direction, say 
	%DIF <  	in $ x^1 
	%DIF <  	$, without lose of generality.}
	%DIF <  \begin{equation}\label{eqn.quasi.lin}
	%DIF <  	\pd{t} \bm{Q} + \bm{M}^k(\bm{Q})\pd{k} \bm{Q} = \bm{S}
	%DIF <  \end{equation}
	%DIF <  for 18 unknowns $ \bm{Q}=\{ \Dfin{\sA}{i}, \Bfin{\sA}{i} \} $ then the matrix $ \bm{M} = \xi_k 
	%DIF <  \bm{M}^k $ has all real eigenvalues but only 10 linearly independent eigenvectors already for 
	%DIF <  the 
	%DIF <  flat Minkowski spacetime, that is the 
	%DIF <  system is only 
	%DIF <  weakly hyperbolic. Here, $ \bm{M}^k =
	%DIF <  \pd{}\bm{F}^k/\pd{}\bm{Q} $ is the Jacobian of the $ k $-\textit{th} flux vector.
	%DIFDELCMD < 

%DIFDELCMD < 	
%DIFDELCMD < 	%%%
%DIF <  Evolution equations \eqref{eqn.3+1.D} and \eqref{eqn.3+1.B} for the torsion
	%DIF <  fields have a conservative form (i.e. their differential terms are the
	%DIF <  4-divergence) which is in particular convenient for developing of high-order
	%DIF <  methods for hyperbolic equations.
	%DIF <  %\begin{subequations}\label{eqn.BD.alt0}
	%DIF <  %	\begin{align}
	%DIF <  	%		\pd{t} \Dfin{\sA}{i} + \pd{k}(\shift{i} 
	%DIF <  	%		\Dfin{\sA}{k} - \shift{k}\Dfin{\sA}{i}  - \LCsymb^{ikj} \,
	%DIF <  	%		\Hfin{\sA}{j}) & 
	%DIF <  	%		= \shift{i}\rho_\sA - \stress{\sA}{i},
	%DIF <  	%		\label{eqn.BD.alt0.D}
	%DIF <  	%		\\[2mm]
	%DIF <  	%		%		
	%DIF <  	%		\pd{t} \Bfin{\sA}{i} + \pd{k}(\,\shift{i} 
	%DIF <  	%		\Bfin{\sA}{k} - \shift{k}\Bfin{\sA}{i}  + \LCsymb^{ikj} 
	%DIF <  	%		\Efin{\sA}{j}) & 
	%DIF <  	%		= 0.
	%DIF <  	%		\label{eqn.BD.alt0.B}
	%DIF <  	%	\end{align}
	%DIF <  %\end{subequations}
	%DIF <  However, this flux-conservative form of equations has a flaw in the
	%DIF <  eigenvalues already for the flat spacetime with non-zero shift vector $
	%DIF <  \shift{k} $. Namely, some characteristic velocities are always $ 0 $ despite
	%DIF <  the non-zero shift vector. The same issue is well known for the
	%DIF <  magnetohydrodynamics equations \cite{Powell1999}. From our experience with
	%DIF <  the non-relativistic equations of nonlinear electrodynamics of moving media
	%DIF <  \cite{DPRZ2017} and continuum mechanics with torsion \cite{Torsion2019} that
	%DIF <  have the same structure as \eqref{eqn.3+1.summary}. Instead, the following
	%DIF <  non-conservative form does not have such a pathology
	%DIF <  \begin{subequations}\label{eqn.BD.alt}
	%DIF <  	\begin{align}
	%DIF <  		\pd{t} \Dfin{\sA}{i} + \pd{k}(\shift{i} 
	%DIF <  		\Dfin{\sA}{k} - \shift{k}\Dfin{\sA}{i}  - \LCsymb^{ikj} \,
	%DIF <  		\Hfin{\sA}{j}) 
	%DIF <  		-\shift{i}\pd{k} \Dfin{\sA}{k}
	%DIF <  		& 
	%DIF <  		= - \stress{\sA}{i},
	%DIF <  		\label{eqn.BD.alt.D}
	%DIF <  		\\[2mm]
	%DIF <  		%		
	%DIF <  		\pd{t} \Bfin{\sA}{i} + \pd{k}(\,\shift{i} 
	%DIF <  		\Bfin{\sA}{k} - \shift{k}\Bfin{\sA}{i}  + \LCsymb^{ikj} 
	%DIF <  		\Efin{\sA}{j}) 
	%DIF <  		-\shift{i}\pd{k} \Bfin{\sA}{k}
	%DIF <  		& 
	%DIF <  		= 0,
	%DIF <  		\label{eqn.BD.alt.B}
	%DIF <  	\end{align}
	%DIF <  \end{subequations}
	%DIF <  and which is obtained from \eqref{eqn.3+1.D} and \eqref{eqn.3+1.B} by
	%DIF <  turning the momentum $ \rho_\sA $ on the right hand-side of
	%DIF <  \eqref{eqn.3+1.D} into a differential term according to the momentum
	%DIF <  constraint $ \rho_\sA = \pd{k}\Dfin{\sA}{k} $ and adding formally a zero, $
	%DIF <  0 = \pd{k}\Bfin{\sA}{k} $, to \eqref{eqn.3+1.B}. These modifications are
	%DIF <  legitimate for smooth solutions thanks to involution constraints
	%DIF <  \eqref{eqn.div.constrD} and \eqref{eqn.Ja0}. After this, the characteristic
	%DIF <  velocities have the desired values equal to $ \sim\shift{k} $ or $ \sim
	%DIF <  \shift{k} \pm c $, if $ c $ is the light speed. However, this only allows to
	%DIF <  fix the issue with the eigenvalues, while some eigenvectors are still
	%DIF <  missing.
	%DIFDELCMD < 
...
%DIFDELCMD < 	
%DIFDELCMD < 	%%%
\DIFdelend Now, rewriting the 3+1 TEGR equations on $\Dfin{\sA}{i}$ and $\Bfin{\sA}{i}$
	...
	rather straightforward sequence of transformations, one obtains the following equations
		\begin{subequations}\DIFdelbegin %DIFDELCMD < \label{eqn.KN.pde}
%DIFDELCMD < 		%%%
\DIFdelend \DIFaddbegin \label{eqn.KN.pde0}
		\DIFaddend \begin{align}
			\DIFaddbegin \DIFadd{\frac{1}{\lapse}}\DIFaddend \pd{\indalg{0}} \Kbuch{\sA}{\sB}
			&
			- \DIFdelbegin \DIFdel{\alpha }\DIFdelend \mg{\sA\sC}\LCtens^{\sC\sD\sE} \pd{\sD} \Nbuchdown{\sE}{\sB}
			- \DIFdelbegin \DIFdel{\alpha }\DIFdelend \pd{\sA} a_{\sB}  \DIFaddbegin \DIFadd{+ }\mg{\sA\sB} \pd{\sC} \DIFadd{n^\sC }\DIFaddend = \text{l.o.t.}
			\\[2mm]
			\DIFaddbegin \DIFadd{\frac{1}{\lapse}}\DIFaddend \pd{\indalg{0}} \Nbuchdown{\sA}{\sB}
			&
			+ \DIFdelbegin \DIFdel{\alpha }\DIFdelend \mg{\sB\sC}\LCtens^{\sC\sD\sE} \pd{\sD} \Kbuch{\sE}{\sA}
			+ \DIFdelbegin \DIFdel{\alpha }\DIFdelend \pd{\sA} \omega_{\sB} = \text{l.o.t.}
			\label{eqn.KN.pde.N}
		\end{align}
	\end{subequations}
	where 'l.o.t.' stands for 'low-order terms' (i.e. algebraic terms that do
	not contain space and time derivatives)\DIFdelbegin \DIFdel{, $ \pd{\indalg{0}} =
	\itetr{\mu}{\indalg{0}}\pd{\mu} = u^\mu\pd{\mu} = \lapse^{-1}\pd{t} -
	\lapse^{-1}\shift{k}\pd{k}$, }\DIFdelend \DIFaddbegin \DIFadd{.
}

	\DIFadd{Finally, using the Hamiltonian constraint \eqref{eqn.Hamiltonian.ERWBB}, the
	...
	for $\Dfin{\sA}{i}$ }\DIFaddend and \DIFdelbegin \DIFdel{$ \pd{\sA} = \itetr{k}{\sA}\pd{k} $. }\DIFdelend \DIFaddbegin \DIFadd{$\Bfin{\sA}{i}$ become
	}\begin{subequations}\label{eqn.KN.pde}
		\begin{align}
			\DIFadd{\pd{\indalg{0}} \Kbuch{\sA}{\sB}
			}&
			\DIFadd{- \alpha \mg{\sA\sC}\LCtens^{\sC\sD\sE} \pd{\sD} \Nbuchdown{\sE}{\sB}
			- \alpha \pd{\sA} a_{\sB} = \text{l.o.t.}\label{eqn.KN.pde.K}
			}\\[2mm]
			\DIFadd{\pd{\indalg{0}} \Nbuchdown{\sA}{\sB}
			}&
			\DIFadd{+ \alpha \mg{\sB\sC}\LCtens^{\sC\sD\sE} \pd{\sD} \Kbuch{\sE}{\sA}
			+ \alpha \pd{\sA} \omega_{\sB} = \text{l.o.t.}
			\label{eqn.KN.pde.N}
		}\end{align}
	\end{subequations}
	\DIFaddend We note the opposite order of the subscripts $\sA$ and $\sB$ in
	\eqref{eqn.KN.pde.N} in the second term with respect to
	\cite[eq.(40)]{Buchman2003}. This \DIFdelbegin \DIFdel{however }\DIFdelend \DIFaddbegin \DIFadd{is solely conditioned by our initial
	choice of ordering the spacetime and tetrad indices in $\Bfin{\sA}{i}$ and
	it }\DIFaddend does not affect the symmetric hyperbolicity of the system discussed
	\DIFdelbegin \DIFdel{in what follows.
%DIF < The low-order terms are given in the appendix\,\ref{}.
	...
	in \cite{Estabrook1997,Buchman2003}, if coupled with the \DIFdelbegin \DIFdel{following }\DIFdelend \DIFaddbegin \DIFadd{Nester of Lorentz
	}\DIFaddend gauge conditions on $a_\sA$ and $\omega_\sA$ \DIFaddbegin \DIFadd{having the form}\DIFaddend :
	\begin{subequations}\label{eqn.aw.pde}
		\begin{align}
			\pd{\indalg{0}} a_\sA - \alpha \MG{\sB\sC}\pd{\sB} \Kbuch{\sC}{\sA} &= \DIFdelbegin \DIFdel{0}\DIFdelend \DIFaddbegin \DIFadd{\text{l.o.t.}}\DIFaddend ,
			\\[2mm]
			\pd{\indalg{0}} \omega_\sA + \alpha \MG{\sB\sC}\pd{\sB} \Nbuchdown{\sC}{\sA} &= \DIFdelbegin \DIFdel{0}\DIFdelend \DIFaddbegin \DIFadd{\text{l.o.t.}}\DIFaddend ,
		\end{align}
	\end{subequations}
	...
	In summary, the 3+1 \tegr\ equations in vacuum, eqs. \eqref{eqn.3+1.D}\DIFdelbegin \DIFdel{and
	\eqref{eqn.3+1.B}, }\DIFdelend \DIFaddbegin \DIFadd{,
	\eqref{eqn.3+1.B}, and \eqref{eqn.3+1.h}, }\DIFaddend if coupled with \DIFdelbegin \DIFdel{the }\DIFdelend proper gauge
	conditions on $A_k$ and $\Omega_k$, are equivalent to \DIFdelbegin \DIFdel{a }\DIFdelend symmetric-hyperbolic
	system \eqref{eqn.KN.pde}--\eqref{eqn.aw.pde} \DIFdelbegin \DIFdel{. However, this is a very preliminary
	result for further investigation of the well-posedness of
	the entire }\DIFdelend \DIFaddbegin \DIFadd{on solutions satisfying the
	...
	\DIFadd{Finally, it is worth remarking about the matter coupling. As we will
	...
	derived for empty spacetimes \mbox{%DIFAUXCMD
\cite{Estabrook1997,Buchman2003}}\hskip0pt%DIFAUXCMD
, we can assume
	that the matter stress tensor will appear on the right-hand side of
	\eqref{eqn.KN.pde.K} as a source term, similar to the standard }\DIFaddend 3+1
	\DIFdelbegin \DIFdel{TEGR
	system because even the time evolution on the tetrad field $\tetr{\sA}{i}$
	was excluded from the consideration}\DIFdelend \DIFaddbegin \DIFadd{decomposition of the Einstein equations (see
	\mbox{%DIFAUXCMD
\cite[eq.(5.19)]{Gourgoulhon2012a}}\hskip0pt%DIFAUXCMD
	...
	\DIFadd{However, if the matter sector is retained, extra equations (such as
	...
	ideal fluids are themselves symmetric hyperbolic \mbox{%DIFAUXCMD
\cite{Ruggeri1981Euler}}\hskip0pt%DIFAUXCMD
,
	there is still hope that the combined system for gravity and matter could be
	symmetrized, at least in this simple case}\DIFaddend .


	
	%DIF <  \textbf{REWRITE THIS!!!!!}  However, we can not immediately conclude the
	%DIF <  same for the original 3+1 TEGR equations \eqref{eqn.3+1.summary} because
	%DIF <  the differential operators of \eqref{eqn.3+1.summary} and
	%DIF <  \eqref{eqn.BD.alt2} are different even though the overall equations are
	%DIF <  equivalent. We recall that the hyperbolicity (well-posedness of the initial
	%DIF <  value problem) is defined by the leading-order terms of the differential
	%DIF <  operator (all differential terms in the case of first-order systems).
	%DIF <  Nevertheless, this is an important result demonstrating that inside of the
	%DIF <  class of equations equivalent to TEGR equations \eqref{eqn.3+1.summary}
	%DIF <  there is a possibility, at least one given by \eqref{eqn.BD.alt2},  to
	%DIF <  rewrite the TEGR vacuum equations as a symmetric hyperbolic system.
	\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < 	%%%
%DIF < The second step to recovering hyperbolicity of the TEGR equations consists in not modifying 
	%DIF < the 
	%DIF < equations but extending the set of state variables and the set of evolution equations.
	%DIF < Thus, following to \cite{Estabrook1997,Buchman2003}, we chose also to evolve the gauge 
	%DIF < freedoms $ 
	%DIF < A_k = 
	%DIF < \Efin{\indalg{0}}{k} $ and $ \Omega_k = \Hfin{\indalg{0}}{k} $ according to
	%DIF < \begin{subequations}\label{eqn.AOmega}
	%DIF < 	\begin{align}
		%DIF < 		&\pd{\indalg{0}}A_i      - \pd{k} \Efinn{k}{i} = 0, 
		%DIF < 		\\[2mm]
		%DIF < 		&\pd{\indalg{0}}\Omega_i + \pd{k} \Hfinnmix{i}{k} = 0.
		%DIF < 	\end{align}
	%DIF < \end{subequations}
	%DIF < We set the right hand-side of these two equations to zero because it is not important for the 
	%DIF < hyperbolicity. In general, one may want to chose any other gauge conditions, e.g. see 
	%DIF < \cite{Buchman2003}. While increasing number of evolution equations, this modification 
	%DIF < drastically 
	%DIF < improves the situation with the missing eigenvectors. Now, the system of 24 equations 
	%DIF < \eqref{eqn.3+1.D}, 
	%DIF < \eqref{eqn.3+1.B}, and \eqref{eqn.AOmega} has only 2 eigenvectors missing, see the 
	%DIF < supplementary 
	%DIF < materials [??].
	%DIFDELCMD < 
...
%DIFDELCMD < 	
%DIFDELCMD < %%%
%DIF < 	\section{Symmetrization}
%DIF < 	
%DIF < 	The energy is
%DIF < 	\begin{equation}
%DIF < 		\Ufin(\tetr{\sA}{k},\Dfin{\sA}{k},\Bfin{\sA}{k}) = 
%DIF < 		-\frac{\alpha}{2\detTetr_3} \left( 
%DIF < 		\varkappa \left( \Dfin{\sB}{\sA} \Dfin{\sA}{\sB} - \frac{1}{2} 
%DIF < (\Dfin{\sA}{\sA})^2\right)
%DIF < 		+
%DIF < 		\frac{1}{\varkappa} \left( \Bfinmix{\sA}{\sB} \Bfinmix{\sB}{\sA} - 
%DIF < \frac{1}{2} 
%DIF < 		(\Bfinmix{\sA}{\sA})^2
%DIF < 		\right)
%DIF < 		\right).
%DIF < 	\end{equation}
%DIF < 	
%DIF < 	Having this expression for the potential $ \Ufin $ and introducing notations
%DIF < 	\begin{equation}
%DIF < 		\Hfinn{\sA}{k} = \frac{\partial \Ufin\hfill}{\partial \Bfin{\sA}{k}},
%DIF < 		\qquad
%DIF < 		\Efinn{\sA}{k} = \frac{\partial \Ufin\hfill}{\partial \Dfin{\sA}{k}},	
%DIF < 	\end{equation}
%DIF < 	the constitutive relations \eqref{eqn.H.E} can be rewritten as
%DIF < 	\begin{equation}\label{eqn.H.E.split}
%DIF < 		\Hfin{\sA}{k} := %\Ufin_{\Bfin{\sA}{k}} =
%DIF < 		\Hfinn{\sA}{k}
%DIF < 		{ 
%DIF < 			\color{blue}
%DIF < 			-
%DIF < 			\varkappa^{-1}\detTetr_3 \LCsymb_{kjl} \itetr{j}{\sA} A^l},
%DIF < 		\qquad
%DIF < 		\Efin{\sA}{k} := %\Ufin_{\Dfin{\sA}{k}}=
%DIF < 		\Efinn{\sA}{k}
%DIF < 		{
%DIF < 			\color{blue}
%DIF < 			-
%DIF < 			\varkappa \detTetr_3 \LCsymb_{kjl} 
%DIF < 			\itetr{j}{\sB} \MG{\sA\sB} \Omega^l}.
%DIF < 	\end{equation}
%DIF < 	
%DIF < 	In turn, the equations for $ \Dfin{\sA}{k} $ and $ \Bfin{\sA}{k} $ can be 
%DIF < written as 
%DIF < 	\begin{subequations}\label{eqn.BD.hat}
%DIF < 		\begin{align}
%DIF < 				\pd{t} \Dfin{\sA}{i} + \pd{k}(\shift{i} 
%DIF < 				\Dfin{\sA}{k} - \shift{k}\Dfin{\sA}{i}  - \LCsymb^{ikj} \,
%DIF < 				\Hfinn{\sA}{j}) 
...
%DIF < 				{ + \color{red} \pd{k} 
%DIF < 						\left(
%DIF < 						\itetr{i}{\sA} \hat{A}^k - \itetr{k}{\sA} \hat{A}^i
%DIF < 						\right) }
%DIF < 				-\shift{i}\pd{k} \Dfin{\sA}{k}
%DIF < 				= - \stress{\sA}{i},
%DIF < 				\\[2mm]
...
%DIF < 				{ - \color{red}\pd{k} 
%DIF < 						\left(
%DIF < 						\itetr{i}{\sB} \hat{\Omega}^k - \itetr{k}{\sB} 
%DIF < \hat{\Omega}^i
%DIF < 						\right) \MG{\sA\sB}}
%DIF < 				-\shift{i}\pd{k} \Bfin{\sA}{k}
%DIF < 				= 0,
%DIF < 			\end{align}
%DIF < 	\end{subequations}
%DIF < 	where {\color{blue}$ \hat{A}^i = \varkappa^{-1} \detTetr_3 A^i $, $ 
%DIF < \hat{\Omega}^i = \varkappa 
%DIF < 		\detTetr_3 
%DIF < 		\Omega^i $.} 
%DIF < 	
%DIF < 	\IP{ If we simply set $ A_i = 0 $ and $ \Omega_i = 0 $, this system of 18 
%DIF < equations has 
%DIF < 		\textit{only 10 eigenvectors} for the flat spacetime.}
%DIF < 	
%DIF < 	Currently, the main natural idea, also exploited by Buchman et al 
%DIF < \cite{Buchman2003} (see 
%DIF < 	eq.(44) 
%DIF < 	or (47)), is to add 
%DIF < 	two PDEs for the $ A_k = \Efin{\indalg{0}}{k} $  and $ \Omega_k = 
%DIF < \Hfin{\indalg{0}}{k} $. 
%DIF < 	
%DIF < 	\IP{The 
%DIF < 		question is how to find these PDEs. So far, I tested the following 
%DIF < simple equations:}
%DIF < 	\begin{subequations}
%DIF < 		\begin{align}
%DIF < 				&\pd{t}A_i - \shift{k}\pd{k}A_i + \itetr{k}{\sA} \pd{k} 
%DIF < \Efin{\sA}{i} = rhs, 
%DIF < 				\\[2mm]
%DIF < 				&\pd{t}\Omega_i -\shift{k}\pd{k}\Omega_i + \itetr{k}{\sA} \pd{k} 
%DIF < \Hfin{\sA}{i} = 
%DIF < 		rhs,  
%DIF < 			\end{align}
%DIF < 	\end{subequations}
%DIF < 	or in terms of \eqref{eqn.H.E.split}
%DIF < 	\begin{subequations}\label{eqn.A.Omega}
%DIF < 		\begin{align}
%DIF < 				&\pd{t}A_i - \shift{k}\pd{k}A_i + {\color{red}\itetr{k}{\sA} 
%DIF < \pd{k} \Efinn{\sA}{i}}
%DIF < 				{\color{blue}
%DIF < 					-\itetr{k}{\sA} \pd{k}(	\LCsymb_{ijl} 
%DIF < 					\itetr{j}{\sB} \hat{\Omega}^l) \MG{\sA\sB}
%DIF < 					}
%DIF < 				 = rhs, 
%DIF < 				\\[2mm]
%DIF < 				&\pd{t}\Omega_i -\shift{k}\pd{k}\Omega_i 
%DIF < 				+ 
%DIF < 				{\color{red}\itetr{k}{\sA} \pd{k} 
%DIF < 					\Hfinn{\sB}{i} \MG{\sA\sB}
%DIF < 					} 
%DIF < 				{\color{blue}
%DIF < 					- \itetr{k}{\sA} \pd{k}(\LCsymb_{ijl} \itetr{j}{\sB} 
%DIF < \hat{A}^l) \MG{\sA\sB}
%DIF < 					}= rhs.
%DIF < 			\end{align}
%DIF < 	\end{subequations}
%DIF < 	Now, \eqref{eqn.BD.hat} and \eqref{eqn.A.Omega} is a system for 24 unknowns  
%DIF < $ \{ 
%DIF < 	\Dfin{\sA}{i},\Bfin{\sA}{i},A_i,\Omega_i\} $ which has 22 eigenvectors. 
%DIF < Therefore, 2 are 
%DIF < 	missing.
%DIF < 	
%DIF < 	\IP{The goal is to try to symmetrize \eqref{eqn.BD.hat}, 
%DIF < \eqref{eqn.A.Omega}. The multipliers 
%DIF < 	are 
%DIF < 		$ 
%DIF < 		\Efinn{\sA}{i} $, $ \Hfinn{\sA}{i} $, $ \hat{A}^i $, and $ 
%DIF < \hat{\Omega}^i $}
%DIF < 	
	%DIFDELCMD < 
...
%DIFDELCMD < 	
%DIFDELCMD < 	%%%
\DIFdelend \section{Relation between the torsion and extrinsic curvature}
	...
	\Dfin{\sA}{i} 
	$, one needs to use the \DIFdelbegin \DIFdel{the }\DIFdelend constitutive relationship \eqref{eqn.E} to deduce
%	\begin{align}
...
%	\end{equation}
	...
	\begin{subequations}\label{eqn.TEGR.Lagr}
		...
		\begin{align}\label{eqn.tors.scal0}
			...
			&\frac{1}{4} g^{\beta\lambda} g^{\mu\gamma} g_{\alpha\eta} \Tors{\alpha}{\lambda\gamma}
			\Tors{\eta}{\beta\mu} \\
			&+
			%
			\frac{1}{2} g^{\mu\gamma} \Tors{\lambda}{\mu\beta} \Tors{\beta}{\gamma\lambda} \\
			&- 
			%
	...
	\begin{align}\label{eqn.Tscal.EB}
		...
		\bigg(
		- \frac{1}{2}(\ETmix{\indalg{0}\lambda}{}\ETmix{\indalg{0}}{\ \lambda} -  
		...
		\nonumber
		\\
		&- \frac{1}{2} h^{-2} ( \BTmix{\indalg{0}\lambda}{}\BTmix{\indalg{0}}{\ \lambda}
	...
	\section{Conclusion and discussion}\DIFaddbegin \label{sec.conclusion}
	\DIFaddend 
	...
	TEGR equations, e.g. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Capozziello2021,Pati2022}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Maluf2001a,Capozziello2021,Pati2022}}\hskip0pt%DIFAUXCMD
}\DIFaddend , and we are not aware
	...
	Our derivation started from the action integral \DIFdelbegin \DIFdel{of TEGR with an arbitrary
	Lagrangian denisty }\DIFdelend \DIFaddbegin \DIFadd{with a
	Lagrangian denisty as an arbitrary function of the tetrad fields and their first gradietns}\DIFaddend , and therefore \DIFaddbegin \DIFadd{some elements of our derivation might be
	used for deriving 3+1 equations of }\DIFaddend extensions of TEGR such as $ f(\Tscal)
	$-teleparallel theories \DIFdelbegin \DIFdel{also can be covered }\DIFdelend in the future. After separating the spatial and time
	components of the torsion, the 3+1 governing partial differential equations
	have appeared to have \DIFdelbegin \DIFdel{the same structure as }\DIFdelend \DIFaddbegin \DIFadd{similar structure to the }\DIFaddend equations of nonlinear
	...
	demonstrated that for the \DIFdelbegin \DIFdel{vacuum equations }\DIFdelend \DIFaddbegin \DIFadd{empty space, }\DIFaddend the differential operator of the \DIFaddbegin \DIFadd{3+1
	vacuum TEGR }\DIFaddend equations can be transformed into a different but equivalent
	\DIFdelbegin \DIFdel{form which is }\DIFdelend \DIFaddbegin \DIFadd{quasi-linear form, provided that the Hamiltonian constraint is fulfilled.
	This form is }\DIFaddend equivalent to the symmetric hyperbolic tetrad reformulation of
	GR by Estabrook-Robinson-Wahlquist \cite{Estabrook1997} and Buchman-Bardeen
	\cite{Buchman2003} \DIFaddbegin \DIFadd{if certain type of gauge conditions is imposed on the
	vectors $\Efin{\indalg{0}}{k}$ and $\Hfin{\indalg{0}}{k}$}\DIFaddend . The question of
	...
	machine precision\DIFaddbegin \DIFadd{. Finally, from the theoretical perspective, it would be
	...
	\mbox{%DIFAUXCMD
\cite{li2011d,li2011e,ferraro2018,blagojevic2020,bajardi2025}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
	...
	\begin{equation}
		...
		\qquad
		\TorsConj{a}{\mu\nu} := \frac{1}{2} \left( \Lagtors_{\Tors{a}{\mu\nu}} - 
		\Lagtors_{\Tors{a}{\nu\mu}} 
		\right)
	\end{equation}
	...
	\begin{subequations}
		\begin{align}%\label{key}
			\Hbb{a}{\mu} &=-\frac{1}{2} \left( \LagBE_{\BT{a}{\mu}} + u^\lambda 
			\LagBE_{\BT{a}{\lambda}} 
			u_\mu 
			\right),
			\\
			\Dbb{a}{\mu} &=-\frac{1}{2} \left( \LagBE_{\ET{a}{\mu}} + u_\lambda 
			\LagBE_{\ET{a}{\lambda}} 
			u^\mu 
			\right).
		\end{align}
	...
	\begin{subequations}
		\begin{align}\label{eqn.tors.scal}
			\Tscal_1 = & \frac{1}{4} \mg{ab}g^{\beta\lambda }g^{\mu\gamma 
			}\Tors{a}{\lambda\gamma} \Tors{b}{\beta\mu },\\[2mm]
			%
			\Tscal_2 = & \frac{1}{2} g^{\mu\gamma} \itetr{\lambda}{a} \itetr{\beta}{b} 
			\Tors{a}{\mu\beta} 
	...
	\begin{subequations}
		\begin{align}\label{eqn.tors.scal.TQLC}
			\mathcal{T}_1 &= Q_3 + \frac{1}{2} C_4, \\
			\mathcal{T}_2 &=- Q_1 + Q_4 + L_1 -C_1 + C_2 + \frac{1}{2} C_4, \\[2mm]
	...
	\begin{align}\label{eqn.tors.scal2.Q}
		Q_1 &= -\frac{1}{2} \ET{\indalg{0}}{\alpha} g^{\alpha\beta} \ET{\indalg{0}}{\beta},\\[2mm]
		%
		Q_2 &= \itetr{\alpha}{a} \ET{a}{\alpha} \itetr{\beta}{b} \ET{b}{\beta} = 
		\ET{\alpha}{\alpha} \ET{\beta}{\beta},\\[2mm]
		%
		Q_3 &=-\frac{1}{2} \mg{ab} \ET{a}{\alpha} g^{\alpha\mu}
		\ET{b}{\mu},\\[2mm]
		%
		Q_4 &=-\frac{1}{2} \itetr{\lambda}{a} \ET{a}{\beta} \itetr{\beta}{b} \ET{b}{\lambda} = 
	...
	\begin{align}\label{eqn.tors.scal.C}
		C_1 &=-\frac{1}{2} h^{-2} g_{\lambda\rho} \BT{\indalg{0}}{\rho} \BT{\indalg{0}}{\lambda},\\[2mm]
		%
		C_2 &=-\frac{1}{2} h^{-2} g_{\rho\varphi} \itetr{\varphi}{a} \BT{a}{\rho} \itetr{\beta}{b} 
