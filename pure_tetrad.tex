...
\begin{document}
	...
	\cite{AldrovandiPereiraBook,Krssak2019} which is know to pass all standard tests of GR. 
	
	From the numerical view point, a proposed $ 3+1 $ formulation has to have well-posed initial 
	value 
	...
	%	\begin{align}
		...
		%	\end{align}
	%\end{subequations}
	with the total (matter+gravity) momentum $ \rho_i := \rho^\text{(m)}_i + \rho^\text{(g)}_i $ 
	given 
	by
	\begin{equation*}
...
%	\end{equation}
	...
	version, 
	i.e. with the spin connection set to zero. To the best of our knowledge there were no attempt 
	to 
