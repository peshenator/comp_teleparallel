...
\newcommand{\Dm}[2]{D_{#1}^{\phantom{#1}#2}}	%Torsion decomposition, analog of Electric field
\newcommand{\aD}[2]{\mathcal{D}_{#1}^{\phantom{#1}#2}}	%Torsion decomposition, analog of 
\newcommand{\Dfin}[2]{\mathtt{D}_{#1}^{\phantom{#1}#2}}	%Torsion decomposition, analog of 
\newcommand{\Hfin}[2]{\mathtt{H}_{#1#2}}	%Torsion decomposition, analog of 
...
\newcommand{\Bfinmix}[2]{\mathtt{b}^{#1}_{#2}}	%Torsion decomposition, analog of Magnetic field
...
\newcommand{\EMmat}[2]{\sigma^{#1}_{\phantom{#1}#2}}
...
general relativity (GR) \cite{Hehl1976,AldrovandiPereiraBook,Cai2016}. In contrast to GR, the main 
dynamical 
field of the teleparallel gravity
is the tetrad (or frame) field and the gravitational 
interaction 
is realized as a curvature-free linear connection of non-zero torsion. In GR on the other hand, the 
main dynamical field 
is the metric tensor while the gravitational interaction is attributed to the curvature 
of a zero-torsion Levi-Civita connection. 
...
the $ 
\hat{0} $-th raw of the frame field for which we standardly assume 
...
we arrive at the final form of the $ 3+1 $ equations for the fields $ \Dfin{a}{i} $ and $ 
\Bfin{a}{i} $
...
\begin{itemize}
	...
	%
	\item
	for the momentum flux, $ \sigma^k_{\ i} = \mat{\sigma^k_{\ i}} + \gra{\sigma^k_{\ 
	i}}$:
	\begin{equation}\label{eqn.sigmaki}
		\gra{\sigma^k_{\ i}} = -\Bfin{a}{k} \Hfin{a}{i} - \Dfin{a}{k} \Efin{a}{i} + 
		\LCsymb_{ilj} 
	...
	\begin{equation}\label{eqn.sigma00}
		\rho_0 :=\sigma^0_{\ 0} = \shift{i} \rho^\text{(m)}_i - \LCsymb_{ilj} 
	...
	%
	\item
	and for the energy flux, $ \sigma^k_{\ 0} = \mat{\sigma^k_{\ 0}}+\gra{\sigma^k_{\ 
	0}}$:
	\begin{equation}\label{eqn.sigmak0}
		\gra{\sigma^k_{\ 0}} = -\beta^j( \Bfin{a}{k} \Hfin{a}{j} + \Dfin{a}{k} \Efin{a}{j})
		+
	...
	\end{equation}
\end{itemize}
It is convenient to split the momentum and energy fluxes $ \sigma^k_{\ i} $ and $ \sigma^k_{\ 0} $ 
in 
...
\begin{subequations}\label{eqn.EM.PDE}
	\begin{align}
		\pd{t}\rho_i & + \pd{k}(-\beta^k \rho_i + s^k_{\ i}) = \rhs{i},\label{eqn.EM.m}\\[2mm]
		\pd{t}\rho_0 & + \pd{k}(-\beta^k \rho_0 + \beta^i s^k_{\ i} + \LCsymb^{klj} \Hfin{a}{l} 
		\Efin{a}{j}) = \rhs{0},\label{eqn.EM.e}
	\end{align}
where $ s^k_{\ i} = \mat{s^k_{\ i}} + \gra{s^k_{\ i}} $ with the gravity part given by
\begin{gather}\label{eqn.flux.const}
	\gra{s^k_{\ i}} := -\Bfin{a}{k} \Hfin{a}{i} - \Dfin{a}{k} \Efin{a}{i} + (\Bfin{a}{j} 
	\Hfin{a}{j} + 
	\Dfin{a}{j} \Efin{a}{j} 
	- \Ufin)\delta^k_{\ i}.
\end{gather}
...
\begin{equation}\label{eqn.J.new}
	\NC{\sA}{k} = \itetr{i}{\sA} \EMmat{k}{i} = \itetr{i}{\sA} (-\shift{k} \rho_i + s^k_{\ i} ) = 
	-\shift{k} \rho_{\sA} + s^k_{\ \sA}.
\end{equation}
...
%		) \KD{\mu}{\nu}
%		\right) & = 0,
		\pd{t}\rho_i + \pd{k}(-\beta^k \rho_i + s^k_{\ i}) &= \rhs{i},
		\label{eq.3+1.m}
		\\[2mm]
		\pd{t}\rho_0 + \pd{k}(-\beta^k \rho_0 + \beta^i s^k_{\ i} + \LCsymb^{klj} \Hfin{a}{l} 
		\Efin{a}{j}) &= \rhs{0},	
...
\begin{gather*}
	s^k_{\ i} := \mat{s^k_{\ i}} + \gra{s^k_{\ i}} = \mat{s^k_{\ i}} -\Bfin{a}{k} \Hfin{a}{i} - 
...
%	+ \rho_k\pd{i}\shift{k} + s^k_{\ j} \w{j}{ik},\\[2mm]
	...
	+ \LCsymb^{klj} \Hfin{\sA}{l}\Efin{\sA}{j}\pd{k}\ln(\alpha) + s^k_{\ \sA} 
	\pd{k}\shift{\sA}\\[2mm]
	=&
	- \left(\pd{t} \ln(\alpha) - \shift{k} \pd{k}\ln(\alpha) \right) \Ufin
	+ \rho_j \pd{t} \shift{j} + s^k_{\ j}\pd{k}\shift{j} + \rho_{\sA} \shift{j} \Efin{\sA}{j} 
	+ \LCsymb^{klj} \Hfin{\sA}{l}\Efin{\sA}{j}\pd{k}\ln(\alpha) + \shift{j} s^k_{\ l}\w{l}{kj},
\end{align*}
...
\begin{subequations}
	\begin{align}\label{eqn.H.E}
		\Hfin{\sA}{k} := %\Ufin_{\Bfin{\sA}{k}} =
		-\frac{\alpha}{\detTetr_3 \varkappa} 
		\mg{\sA\sB}\mg{\sC\sD} 
		...
		\Dfin{\sB}{j} 
		- 
		\frac{\varkappa}{\detTetr_3} \LCsymb^{jil} 
		\gamma_{jk} \tetr{\sA}{i} \Hfin{\indalg{0}}{l}.
	\end{align}
\end{subequations}
...
\subsubsection{Stationary differential constraints}
	...
	\begin{equation}\label{eq.div.constr2}
		\pd{k} \Dfin{\indalg{0}}{k} = -\alpha^{-1} \Ufin,
		\qquad
...
%=======
%\begin{equation}\label{eq.D0}
%	\aD{\indalg{0}}{0} = \frac{1}{2\alpha} \left(  \aD{k}{k} - \frac{\detTetr_3}{2\alpha
%	\varkappa} 
%	\beta^k 
%	N_k \right),
%	\qquad
%	\aD{\indalg{0}}{k} = -\beta^k \aD{\indalg{0}}{0} + \frac{\detTetr_3}{\varkappa} \LCsymb_{jli} 
%	\gamma^{jk}\aB{l}{i},
%	\qquad
%	\aD{\sA}{0} = -\frac{\detTetr_3}{2\alpha\varkappa}\itetr{k}{\sA} N_k.
%\end{equation}
%
%\begin{equation}\label{eq.D0.tmp}
%	\Dm{\indalg{0}}{0} = \frac{1}{2 \alpha} \left(  \Dm{k}{k} - \frac{\detTetr_3}{2 
%		\varkappa} 
%	\beta^k 
%	N_k \right),
%	\qquad
%	\Dm{\indalg{0}}{k} = -\beta^k \Dm{\indalg{0}}{0} - \frac{\detTetr_3}{\varkappa} \LCsymb_{jli} 
%	\gamma^{jk}\Bm{l}{i},
%	\qquad
%	\Dm{\sA}{0} = -\frac{\detTetr_3}{2\varkappa}\itetr{k}{\sA} N_k.
%\end{equation}
%
%\begin{equation}\label{eq.D0.tmp2}
%	\Dm{\indalg{0}}{0} = \frac{1}{2 \alpha} \tetr{\sA}{k}\left(  \Dm{\sA}{k} + 
%	\beta^k \Dm{\sA}{0} \right),
%	\qquad
%	\Dm{\indalg{0}}{k} = -\beta^k \Dm{\indalg{0}}{0} - \frac{\detTetr_3}{\varkappa} \LCsymb_{jli} 
%	\gamma^{jk}\Bm{l}{i},
%	\qquad
...
%>>>>>>> bebd74bc21b3427295a03679c89de9dd23ef69b2
...
\begin{subequations}
	\begin{equation}\label{eqn.Ufin}
		\Ufin(\tetr{\sA}{k},\Dfin{\sA}{k},\Bfin{\sA}{k}) = \frac{\alpha}{2\detTetr_3} \left( 
		\varkappa \left( \frac{1}{2} I_{\Dfin{}{}1} -I_{\Dfin{}{}2}\right)  
		+
		\frac{1}{\varkappa} \left( \frac{1}{2} I_{\Bfin{}{}1} -I_{\Bfin{}{}2}
		\right)
...
\begin{subequations}
	\begin{align}
		\pd{t} \rho_\sA + & \pd{k} (-\shift{k} \rho_\sA + s^k_{\ \sA}) = 0,\\[2mm]
...
\section{Some analytical relations to GR}

In GR, the evolution equation of the spatial conformal metric $ \tilde{\gamma}_{ij} = 
...
\end{equation} 
where $ \phi = \det(\gamma_{ij})^{-1/6} = \detTetr_3^{-1/3} $, $ K_{ij} $ is the spatial extrinsic 
curvature, and $ K = K_{ij}\gamma^{ij} $ is its trace.
...
\begin{equation}\label{eqn.gamma.PDE}
	...
	\Efin{\sB}{i})	.
\end{equation} 
Then, using the evolution equation of the determinant $ \detTetr_3 = \sqrt{\det(\gamma_{ij})} $
\begin{equation}\label{eqn.h.PDE}
	\pd{t}\detTetr_3 - \pd{k}(\detTetr_3\shift{k} ) = \frac{\alpha \varkappa}{2} \Dfin{\sA}{\sA},
\end{equation}
...
\end{equation}
Hence, the first outcome is that in order TEGR and GR have the same solutions, one at least has to 
impose that
\begin{equation}
	\Dfin{\sA}{\sA} = \tetr{\sA}{i} \Dfin{\sA}{i} = -2 \frac{K \detTetr_3}{\varkappa},
\end{equation}
while the second outcome 
\begin{equation}
	\mg{\sA\sB} \bigg( 
	...
	\bigg) \Dfin{\sC}{k} = \frac{2 \detTetr_3}{\varkappa} K_{ij}
\end{equation}
follows from the fact that
\begin{equation}
	\mg{\sA\sB} (\tetr{\sA}{i}\Efin{\sB}{j} + \tetr{\sA}{j}\Efin{\sB}{i})
	=
	\mg{\sA\sB} \frac{\alpha \varkappa}{\detTetr_3}\bigg( 
	\tetr{\sB}{k} (\tetr{\sA}{i}\tetr{\sC}{j} + \tetr{\sA}{j}\tetr{\sC}{i})
	-
	\frac12
	(\tetr{\sA}{i}\tetr{\sB}{j} + \tetr{\sA}{j}\tetr{\sB}{i})\tetr{\sC}{k}
