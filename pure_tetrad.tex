...
\begin{equation}\label{eqn.KN}
	...
	\qquad
	\Nbuch{\sB}{\sA} := \frac{1}{2} \epsilon^{\sA\sC\sD}\mathcal{R}_{\sC\sD\sB}
\end{equation}
and 
\begin{equation}\label{eqn.aw}
	a_{\sA} = \mathcal{R}_{\sA\indalg{0}\indalg{0}},
	\qquad
	\omega^{\sA} = \frac{1}{2} \epsilon^{\sA\sB\sC} \mathcal{R}_{\sC\sB\indalg{0}}
\end{equation}
...
\lapse^{-1}\pd{t} - 
\lapse^{-1}\shift{k}\pd{k}$ and $ \pd{\sA} = \itetr{k}{\sA}\pd{k} $. Also, $ \LCtens_{\sA\sB\sC} = 
\detTetr_3 \LCsymb_{ijk}\itetr{i}{\sA}\itetr{j}{\sB}\itetr{k}{\sC} $. The low-order terms are given 
in the appendix\,\ref{}.
The constitutive fluxes $ \Efin{}{\sA\sB} $ and $ \Hfin{\sB}{\sA} $ in terms of $ 
\{\Kbuch{\sA}{\sB},\Nbuchdown{\sA}{\sB},a_{\sA},\omega_{\sA}\} $ are given by 
...
in such a way so that to guaranty the well-posedness of the enlarged system for the unknowns $ 
\{\Kbuch{\sA}{\sB},\Nbuchdown{\sA}{\sB},a_\sB,\omega_\sB \}$. Thus, as shown in 
\cite{Estabrook1997,Buchman2003}, if coupled with equations whose the differential part is
\begin{align}\label{eqn.aw.pde}
	\pd{\indalg{0}} a_\sA - \pd{\sB} \Kbuch{\sB}{\sA} &= 0,
	\\[2mm]
	\pd{\indalg{0}} \omega_\sA + \pd{\sB} \Nbuchdown{\sB}{\sA} &= 0,
\end{align}
system \eqref{eqn.KN.pde} is symmetric hyperbolic.
Therefore, we may conclude the same for the TEGR equations \eqref{eqn.BD.alt2}.
