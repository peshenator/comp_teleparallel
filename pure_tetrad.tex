...
\begin{subequations}\label{eqn.KN.pde}
	...
	\end{align}
\end{subequations}
where 'l.o.t.' stands for 'low-order terms' (or algebraic terms that do not contain space and time 
derivatives) $ \pd{\indalg{0}} = \itetr{\mu}{\indalg{0}}\pd{\mu} = u^\mu\pd{\mu} = 
\lapse^{-1}\pd{t} - 
\lapse^{-1}\shift{k}\pd{k}$ and $ \pd{\sA} = \itetr{k}{\sA}\pd{k} $. The low-order terms are given 
in the appendix\,\ref{}.
