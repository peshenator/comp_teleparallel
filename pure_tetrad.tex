...
\begin{document}
	...
	space-time the obtained system of first-order equations is equivalent to the tetrad 
	reformulation 
	of general relativity by Estabrook, Robinson, Wahlquist, and Buchman and Bardeen and therefore 
	also 
	admits a symmetric 
	...
	From now on, we shall formally treat the frame field $ \tetr{a}{\mu} $ and its gradients (the 
	\We\ 
	connection) $ 
	\pd{\lambda}\tetr{a}{\mu} $ as independent variables and in what follows, we shall rewrite 
	system 
	...
	Finally, formally extending the shift vector as $ \bm{\beta}=(-1,\beta^{k}) $ and introducing 
	the 
	change of variables
	...
	\LCsymb^{klj} \Dfin{lj}{}) $ to the equation for the $ \Bfin{\sA}{i} $ field. This expression 
	is zero 
	...
	not 
	provided with particular evolution equations and therefore, their governing equations can be 
	chosen 
	...
	%
	%Currently, the main natural idea, also exploited by Buschman et al \cite{Buchman2003} (see 
	%eq.(44) 
	%or (47)), is to add 
	...
	%	\begin{align}
		...
		%		\\[2mm]
		%		&\pd{t}\Omega_i -\shift{k}\pd{k}\Omega_i + \itetr{k}{\sA} \pd{k} \Hfin{\sA}{i} = 
		%rhs,  
		%	\end{align}
	%\end{subequations}
	...
	%\Dfin{\sA}{i},\Bfin{\sA}{i},A_i,\Omega_i\} $ which has 22 eigenvectors. Therefore, 2 are 
	%missing.
	%
	%\IP{The goal is to try to symmetrize \eqref{eqn.BD.hat}, \eqref{eqn.A.Omega}. The multipliers 
	%are 
		%$ 
	...
	curvature.
	On the other hand, contracting \eqref{eqn.tetr.3+1} with $ \mg{\sA\sB} \tetr{\sB}{j} $, one 
	obtains 
