...
\reply{We appreciate the suggestions for relevant references regarding the $f(T)$ gravity and Hamiltonian formulations of teleparallel gravity. They helped us to clarify the differences between the existing 3+1 Hamiltonian formulations and our technique (please see the changes in the introduction). We have added these references in the revised manuscript in relevant places, Section\,3 and conclusion. Apparently, $f(T)$ gravity, as well as the covariant TEGR, will require some modifications to our approach, and we hope to address this in future work.}
\\
...
\reply{This is an important point that forced us revise Section 10. We have found out that it was written misleadingly. In fact, we were not able to establish the exact equivalence of the two formulations but only of the principal parts of the differential operators (all differential terms for a first-order system), which is sufficient for our hyperbolicity analysis. Unfortunatelly, we can not confirm nor disprove the complete equivalence at this stage because the algebraic parts of the systems do not match. This is also stated explicitly in the revised version. After a careful revision, we removed the misleading statements in Section 10, as well as in the abstract, introduction and conclusion. }
\\
...
\reply{To the best of our knowledge, the ERWBB formulation was only considered
in vacuum. Therefore, we cannot make any claims about the equivalence of the 3+1
equations for matter at this stage. However, one could expect that, as in other
3+1 formulations utilizing the extrinsic curvature, the matter source term must
be in their PDE for the $\mathtt{K}_{AB}$ field, which is nothing else but the
extrinsic curvature (up to the sign convention) written in the tetrad frame. If
we keep the matter source in our $\mathtt{D}^i_{\ A}$ equation, then it will be
inherited in the PDE for $\mathtt{K}_{AB}$ as well. We elaborated on this in the
revised manuscript, at the end of Sec.10.} \\
